...
public class MergedRecordListOld extends QueryResultLazyList<Record> {
    ...
    public MergedRecordListOld(NativeSQL nativeSQL, PersistenceUnit persistenceUnit) throws SQLException, UPAException {
        super(nativeSQL);
        updatable = getNativeSQL().isUpdatable();
    ...
    public Record parse(QueryResult result) {
        if (updatable) {
            ...
            record.addPropertyChangeListener(currentPropertyChangeListener= new QueryResultUpdaterPropertyChangeListenerOld(this, result));
