...
public class XLSXPackagePart {
    ...
    public XLSXPackagePart(XLSXFile file, ZipEntry entry, ObjectFactory factory) {
        this.container = file;
        this.entry = entry;
    ...
    public List<XLSXPackageRel> getRelationships() throws IOException, SAXException, InterruptedException {
        ...
        if (relationshipPart != null) {
            StackBlockingSAXReader reader = StackBlockingSAXReaderFactory.createStackBlockingSAXReader(relationshipPart.getInputStream(), 10, new RelationshipProcessor(null, StringUtils.split(path, '/', false)[0]), factory);
