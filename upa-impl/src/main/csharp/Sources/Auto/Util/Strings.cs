...
namespace Net.Vpc.Upa.Impl.Util
{
    ...
    public sealed class Strings {
        ...
        public static System.Collections.Generic.IList<string> ParseVarsList(string s) {
            ...
            System.Text.RegularExpressions.Regex p = new System.Text.RegularExpressions.Regex("\\$\\{[a-zA-Z]+\\w*\\}");
            System.Text.RegularExpressions.Match m = p.Match(s);
            while (m.Find()) {
                int n = 0;
        ...
        public static string ToHexString(byte[] bytes) {
            ...
            for (int i = 0; i < bytes.Length; i++) {
                string hex = ((int)(0xff & bytes[i])).ToString("X");
                if ((hex).Length == 1) {
                    hexString.Append('0');
                }
        ...
        public static bool MatchesSimpleExpression(string str, string pattern) {
            return (new System.Text.RegularExpressions.Regex(SimpexpToRegexp(pattern)).Match(str == null ? "" : str)).Success;
        }
