    ...
    private Map<String, Obj> getNonNullRelations(String entityName, Object instance, PersistenceUnit target) {
        ...
        for (Field field : entity.getFields()) {
            ...
            if (manyToOneRelationship != null) {
                Entity rr = target.getEntity(manyToOneRelationship.getTargetEntity().getName());
                Object v2 = bb.getProperty(instance, field.getName());
                if (v2 != null) {
                    t.put(field.getName(), new Obj(rr.getName(), v2));
                } else {
                    //try foreigns
                    List<Field> rfields = manyToOneRelationship.getSourceRole().getFields();
...
//        Object id = toPrimitiveId(entity.getName(), oo.value, source);
//        Object reloaded = entity.createQueryBuilder().setHint(QueryHints.MAX_NAVIGATION_DEPTH, 1).setId(id).getEntity();
